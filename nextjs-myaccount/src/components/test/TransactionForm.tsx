...
function TransactionForm() {
  ...
  const handleSubmit = async () => {
    ...
    const balance =
      formValues.type === 'withdraw'
        ? account.balance - Number(formValues.amount) //출금
        : account.balance + Number(formValues.amount) //입금

    const newTransaction = {
      ...formValues,
      amount: Number(formValues.amount),
      date: new Date().toISOString(), //date타입을 ISO 포맷 스트링타입으로 변환
      balance,
    } as Transaction
    ...
    await Promise.all([
      //두개가 같이 일어나기 때문에 각각 await을 붙이는것보다는 Promise.all을 사용
